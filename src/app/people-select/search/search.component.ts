...
export class SearchComponent {
  ...
  constructor(sanitizer: DomSanitizer,
    ...
    if (this.local.getObject('searchHistorys')) {
      this.historys = this.local.getObject('searchHistorys')
    }
  }
  ...
  onSumbit() {
    let keyword = StringUtils.trim(this.keyword);
  ...
  private saveSearchKeyword(keyword: string) {
    if (this.historys.findIndex(v => v == keyword) == -1) {
      ...
      this.local.setObject('searchHistorys', this.historys)
    }
  }
