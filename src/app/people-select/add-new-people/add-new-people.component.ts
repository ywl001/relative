...
export class AddNewPeopleComponent {
  ...
  set data(value: People) {
    if (value) {
      ...
      } else if (value.peopleNumber) {
        this.pid = value.peopleNumber
      }
    }
  }

  constructor(sanitizer: DomSanitizer,
    iconRegistry: MatIconRegistry,private sql:SqlService) {
  ...
  onSubmit() {
    if(this.validate()){
      let p = this.getData();
      this.sql.insert('people',p).subscribe(res=>{
        console.log(res);
        if(res>0){
          p.id = res;
  ...
  private getData() {
    let data: any = {
      name: this.pName,
      peopleNumber: this.pid,
      telephone: this.tel,
      sex: this.getSex(),
      isDead: this.isDead ? 1 : 0
    };
    return data;
  }
}
